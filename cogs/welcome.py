...
class WelcomeBackgroundModal(discord.ui.Modal, title="Customise Background"):
...
class WelcomeImageModal(discord.ui.Modal, title="Customise Card Text"):
...
class WelcomeDashboardView(PrivateLayoutView):
        ...
        if is_enabled:
            ...
            if show_image:
                ...
                section_bg = discord.ui.Section(
                    discord.ui.TextDisplay(
                        f"The Welcome Card (image). Use the customise button to upload a custom background."
                    ),
                    accessory=btn_bg_config
                )
                container.add_item(section_bg)

                line1_text = self.data.get("image_line1") or "Welcome {member.display_name}"
                line2_text = self.data.get("image_line2") or "You are our {position} member!"
                ...
                container.add_item(discord.ui.TextDisplay(
                        ...
                        f"  * `{{member.display_name}}`, `{{member.name}}`, `{{server.name}}`, `{{position}}`, and others available in Discord member or server/guild objects."))
    ...
    async def open_background_modal(self, interaction: discord.Interaction):
        await interaction.response.send_modal(WelcomeBackgroundModal(self.background_modal_callback))
...
class Welcome(commands.Cog):
    ...
    def generate_welcome_card(self, member: discord.Member, data: dict, avatar_bytes: Optional[bytes]) -> discord.File:
        ...
        mask1 = get_text_mask(line1_text, 30, font_name="gg sans", weight="Bold")
